...
async function getSemanticTokens(uri: vscode.Uri, document: vscode.TextDocument) {
...
async function getFallbackSymbols(uri: vscode.Uri): Promise<SemanticToken[]> {
    ...
    return symbols.map(symbol => {
        ...
        return {
            ...
            tokenType: (symbol.kind < tokenTypes.length ? tokenTypes[symbol.kind] : 'unknown') as unknown as TokenType | "unknown",
            modifiers: [],
            text: symbol.name
        } as SemanticToken;
    });
...
export const get_semantic_tokens: ToolDef<GetSemanticTokensArgs, GetSemanticTokensResult> = {
    id: "get_semantic_tokens",
    ...
    async run({ textDocument }: GetSemanticTokensArgs) {
        ...
        const { supported, document, error } = await checkSemanticTokensSupport(uri);
        
        if (!supported || !document) {
            return { tokens: [], error };
        }

        const { success, semanticTokens, error: tokenError } = await getSemanticTokens(uri, document);
