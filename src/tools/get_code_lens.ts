...
type GetCodeLensResult = CodeLensItem[] | {
    content: { type: "text"; text: string }[];
    isError: boolean;
};

async function convertCodeLens(lens: vscode.CodeLens, uri: vscode.Uri): Promise<CodeLensItem> {
    return {
...
export const get_code_lens: ToolDef<GetCodeLensArgs, GetCodeLensResult> = {
    ...
    async run({ textDocument }: GetCodeLensArgs) {
        ...
        try {
            ...
            return Promise.all(codeLensResult.map(lens => convertCodeLens(lens, uri)));
        } catch (error) {
            return {
                content: [{ 
                    type: "text", 
